...
本機、不上傳雲端的會議錄音轉逐字稿工具，給 [Claude Code](https://claude.com/claude-code) 用的 skill。用 Apple 的 MLX 框架跑 Whisper + pyannote 做語者辨識，適合國語/英文為主的會議。
...
轉錄用官方 `mlx-community/whisper-large-v3-turbo`，不用另外下載或轉檔，第一次執行時自動抓取。

## 通用詞彙字典（改善專有名詞辨識）

`glossary/web3-glossary.md` 收錄一份通用的 Web3/加密貨幣領域專有名詞（知名交易所、
公鏈、DeFi 協議、公開人物、行業術語），開會內容常提到這類詞時可以用
`--initial-prompt` 餵給模型，改善辨識率——公司名、人名、產品名這類詞光靠模型
自己聽很容易錯（同音字亂猜、英文專有名詞音譯錯）。

**這份字典只收公開、通用的產業詞彙**，不收任何特定公司內部的人名/產品名。如果
你有自己的私有詞彙（公司內部資訊），另外存在私有的地方，不要放進這個公開 repo，
組 `--initial-prompt` 時把公開字典跟你自己的私有字典內容合併即可。
...
`setup.sh` 會自動建立獨立的 conda 環境 `whisper`（Python 3.11，避開新版 Python 相依套件缺 wheel 的問題），並安裝 `mlx-whisper`、`opencc-python-reimplemented`、`pyannote.audio`。
